...
import { ROUTE_BlOG, ROUTE_CATEGORY_NEW, WEB_DOMAIN } from "@ltp/utils/constant";
...
export const PostPage = ({ news, related, hot }) => {
  ...
  const translates_category = Lodash.get(news, "category_obj.translates", []);
  const data_category = translates_category.reduce((obj, item) => {
    if (item.language_code === language) {
      return {
        ...obj,
        [item.language_field]: item.language_value,
        ...
        <Box px={{ base: 5, md: 40 }} marginBottom="116px">
          ...
          >
            <Breadcrumb>
              ...
              <BreadcrumbItem>
                <a href={addTrailingSlash(ROUTE_CATEGORY_NEW(language))}>
              ...
              >
                <a
                  href={addTrailingSlash(
                    concatUrls(WEB_DOMAIN, ROUTE_BlOG(language, getValidSlug(data_detail))),
                  )}
                >
