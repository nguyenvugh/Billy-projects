...
@Injectable()
export class FacebookAuthenticateDriver implements AuthenticateDriverInterface {
  ...
  async verify(data: any): Promise<AuthenticateResultInterface> {
    ...
    const { oauthAccessToken, uid } = data;
    if (!oauthAccessToken || !uid) {
    ...
    return new Promise((resolve) => {
      try {
        const credential =
          firebase.auth.FacebookAuthProvider.credential(oauthAccessToken);
        firebase
          .auth()
          ...
          .then((rs) => {
            ...
            if (user && user.uid == uid) {
              ...
              result.uid = user.uid;
              result.email = user.providerData[0].email;
